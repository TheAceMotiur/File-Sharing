...
class AdminController extends Controller
{
    ...
    public function cacheStats()
    {
        ...
        foreach ($files as $file) {
            if (is_file($file)) {
                ...
                $stmt = $db->prepare("SELECT original_name FROM file_uploads WHERE unique_id = ?");
                ...
                $cacheFiles[] = [
                    'uniqueId' => $uniqueId,
                    'fileName' => $fileInfo['original_name'] ?? 'Unknown',
                    'size' => $this->formatBytes($size),
                    'age' => $this->formatAge($age),
                    'lastAccess' => date('Y-m-d H:i:s', $mtime)
                ];
            }
        }
        ...
        usort($cacheFiles, function($a, $b) {
            return strtotime($b['lastAccess']) - strtotime($a['lastAccess']);
        });
        
        $avgAge = $fileCount > 0 ? $totalAge / $fileCount : 0;
        
        echo json_encode([
            'success' => true,
            'totalSize' => $this->formatBytes($totalSize),
            'fileCount' => $fileCount,
            'avgAge' => $this->formatAge($avgAge),
            'hitRate' => 'N/A',
            'files' => $cacheFiles
        ]);
    }
    ...
    public function cacheClear()
    {
        ...
        echo json_encode([
            'success' => true,
            'message' => "Cleared {$removed} cached file(s)"
        ]);
    }
    ...
    public function cacheDelete()
    {
        ...
        $cacheFile = __DIR__ . '/../../cache/' . $uniqueId;
